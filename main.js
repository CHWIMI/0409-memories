...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 브라우저 탭 전환(비활성) 시에도 재생 중인 비디오를 정지
    ...
    window._isMinigameOpen = false; // 미니게임 진행 중 플래그 (minigame.js와 공유)

    // idle 감지: 유저 활동 시 타이머 리셋
    function resetIdleTimer() {
        // 미니게임 모달이 열려 있는 동안에는 idle 타이머를 건드리지 않음
    ...
    function spawnFooterWalker() {
        ...
        walker.src = 'img/egg.gif';
        walker.alt = '';
        walker.classList.add('egg-walker', 'footer-walker');
        ...
        // 모아를 클릭/터치하면 미니게임 발동 (idle 감지로 전파 차단)
        let walkerCaught = false;
        function catchWalker(e) {
            if (walkerCaught) return; // 중복 호출 방지
            walkerCaught = true;
            e.stopPropagation();
            e.preventDefault();
            // walker 애니메이션 정지
            ...
            // 멈칫 연출: 현재 위치에서 멈추고 살짝 깜짝 표현
            const computed = window.getComputedStyle(walker);
            walker.style.transition = 'none';
            walker.style.left = computed.left;

            // 살짝 위로 튀는 깜짝 연출
            requestAnimationFrame(() => {
                walker.style.transition = 'transform 0.25s ease';
                walker.style.transform = 'translateY(-12px) scale(1.15)';
                setTimeout(() => {
                    // 페이드아웃 후 미니게임 열기
                    walker.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
                    walker.style.opacity = '0';
                    walker.style.transform = 'translateY(-20px) scale(0.8)';
                    setTimeout(() => {
                        walker.remove();
                        footerWalkerEl = null;
        ...
        // touchstart에서 잡아야 모바일에서 idle dismiss보다 먼저 동작함
        walker.addEventListener('touchstart', (e) => {
            e.stopPropagation();
            catchWalker(e);
        }, { passive: false });
        walker.addEventListener('click', catchWalker);
    ...
    const closeMinigameBtn = document.querySelector('.close-minigame-modal');
    if (closeMinigameBtn) {
        closeMinigameBtn.addEventListener('click', () => {
            if (typeof window.closeMiniGame === 'function') window.closeMiniGame();
            minigameModal.close();
            document.body.style.overflow = 'auto';
    ...
    if (minigameModal) {
        minigameModal.addEventListener('click', (event) => {
        ...
        minigameModal.addEventListener('close', () => {
            document.body.style.overflow = 'auto';
            window._isMinigameOpen = false;
            if (typeof window.closeMiniGame === 'function') window.closeMiniGame();
            clearTimeout(idleTimer);
    ...
    /* --- Easter Egg #4: Billboard Idle (5 mins) --- */
    let billboardIdleTimer = null;
    let isBillboardSecretActive = false;
    let isBillboardSecretPreloaded = false;

    function resetBillboardIdleTimer() {
        const secretImg = document.getElementById('secret-submain-img');
        if (!secretImg) return;

        clearTimeout(billboardIdleTimer);

        // 시크릿 이미지가 떠 있다면 원상 복구 (크로스페이드 아웃)
        if (isBillboardSecretActive) {
            secretImg.classList.remove('revealed');
            isBillboardSecretActive = false;
        }

        // 5분(300,000ms) 뒤 이스터에그 발동
        billboardIdleTimer = setTimeout(triggerSecretBillboard, 300000);

        // 딱 10초 전(4분 50초, 290,000ms)에 이미지를 백그라운드에서 다운로드 시작
        if (!isBillboardSecretPreloaded) {
            setTimeout(() => {
                if (!isBillboardSecretPreloaded) {
                    secretImg.src = 'submain.jpg'; // HTML의 src 변경
                    isBillboardSecretPreloaded = true;
                }
            }, 290000);
        }
    }

    function triggerSecretBillboard() {
        const mainPage = document.getElementById('main-page');
        ...
        if (mainPage && !mainPage.classList.contains('hidden') &&
            billboardSection && billboardSection.classList.contains('active') && secretImg) {

            // 혹시 탭 전환 등으로 프리로드가 꼬여서 안 됐을 경우를 대비한 보험
            if (!isBillboardSecretPreloaded) {
                secretImg.src = 'submain.jpg';
                isBillboardSecretPreloaded = true;
            }

            // 약간의 딜레이를 주어 src 할당 후 트랜지션이 부드럽게 먹히도록 함
            setTimeout(() => {
                secretImg.classList.add('revealed');
                isBillboardSecretActive = true;
            }, 50);
        } else {
            // 다른 탭에 있다면 타이머 초기화 후 재개
            resetBillboardIdleTimer();
        }
    }

    // 유휴 상태 감지 이벤트(기존 이벤트 리스트 재사용)
    idleEvents.forEach(evt => window.addEventListener(evt, resetBillboardIdleTimer, { passive: true }));
    // 초기 타이머 시작
    resetBillboardIdleTimer();
    ...
    function preloadGalleryImages() {
        // 이스터에그용 이미지들 미리 캐싱하기
        new Image().src = 'img/egg.gif';
        new Image().src = 'img/eggred.gif';
        new Image().src = 'favicon.png';
        // 기존 Logic
